...
final class LirauOnboardingFlowLorauaRegisterViewController: UIViewController, UITextFieldDelegate, UIGestureRecognizerDelegate {
    ...
    private let nextButton = LirauInteractiveLearningLorauaButton(title: "Next")
    ...
    @objc private func continueToProfile() {
        ...
        switch LirauOnboardingFlowLorauaStore.shared.onboardingFlowLorauaCanRegister(email: emailField.text ?? "", password: password) {
        case .success(let email):
            let controller = LirauProfileIntroLorauaSetupViewController(email: email, password: password)
            controller.onAuthenticated = onAuthenticated
            navigationController?.pushViewController(controller, animated: true)
        case .failure(let error):
            lirauAlertSoundLorauaNotice(title: "Registration failed", message: error.message)
        }
    }
