...
enum LirauCommunityChallengePhase: String, Codable {
    ...
    var legacyStatus: LirauCommunityChallengeStatus {
        switch self {
        case .open: return .speacketLirau
...
enum LirauCommunityChallengeStatus: String, Codable {
    ...
    var phase: LirauCommunityChallengePhase {
        switch self {
        case .speacketLirau: return .open
        case .voting: return .voting
        case .ended: return .ended
        }
    }
    ...
    var title: String {
        switch self {
        case .speacketLirau: return "Live"
        case .voting: return "Voting"
        case .ended: return "Ended"
        }
    }
    ...
    var actionTitle: String {
        switch self {
        case .speacketLirau: return "Join"
    ...
    var foregroundColor: UIColor {
        switch self {
        case .speacketLirau: return UIColor(red: 0.34, green: 1.0, blue: 0.66, alpha: 1)
    ...
    var backgroundColor: UIColor {
        switch self {
        case .speacketLirau: return UIColor(red: 0.12, green: 0.5, blue: 0.28, alpha: 0.24)
...
struct LirauCommunityChallenge: Codable, Equatable {
    ...
    var profileIntroLorauaCurrentUserMode: LirauCommunityUserMode
    var profileIntroLorauaCurrentUserEntryID: String?
    var votedEntryID: String?
    var profileIntroLorauaCurrentUserVotedEntryIDs: [String]?
    ...
    var profileIntroLorauaCurrentUserEntry: LirauCommunityEntry? {
        guard let profileIntroLorauaCurrentUserEntryID else { return nil }
        return entries.first { $0.id == profileIntroLorauaCurrentUserEntryID }
    }
    ...
    func applyingUserState(_ state: LirauCommunityLocalUserChallengeState?) -> LirauCommunityChallenge {
        ...
        var challenge = self
        challenge.profileIntroLorauaCurrentUserMode = state.role.legacyMode
        challenge.profileIntroLorauaCurrentUserEntryID = state.myEntryID
        challenge.votedEntryID = state.votedEntryIDs.first
        challenge.profileIntroLorauaCurrentUserVotedEntryIDs = state.votedEntryIDs
        return challenge
    }

    var recordedVotedEntryIDs: [String] {
        if let profileIntroLorauaCurrentUserVotedEntryIDs {
            return profileIntroLorauaCurrentUserVotedEntryIDs
        }
        if let votedEntryID {
            return [votedEntryID]
        }
...
struct LirauCommunityUserSnapshot {
    ...
    static var current: LirauCommunityUserSnapshot {
        let profile = LirauOnboardingFlowLorauaStore.shared.globalCitizenLorauaCurrentProfile ?? LirauGlobalCitizenLorauaAccountProfile.globalCitizenLorauaTestProfile
        return LirauCommunityUserSnapshot(
            userID: profile.globalCitizenLorauaID.isEmpty ? "lirau_local_learner" : profile.globalCitizenLorauaID,
            displayName: profile.nativeSpeakerLorauaName.isEmpty ? "LiraU Speaker" : profile.nativeSpeakerLorauaName,
            languagePairing: profile.languagePairingLorauaSummary.isEmpty ? "English - Spanish" : profile.languagePairingLorauaSummary
        )
    }
}
