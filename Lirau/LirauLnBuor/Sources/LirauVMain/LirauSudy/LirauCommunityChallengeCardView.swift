...
final class LirauCommunityChallengeCardView: UIControl {
    ...
    private let statusPill = LirauCommunityStatusPill(status: .speacketLirau)
    ...
    func configure(with challenge: LirauCommunityChallenge) {
        ...
        let howshower = challenge.status == .speacketLirau
        let highlightedLike = howshower || challenge.isLikedByCurrentUser
        ...
        if challenge.status == .ended || challenge.profileIntroLorauaCurrentUserMode == .host || challenge.profileIntroLorauaCurrentUserMode == .submitted || !challenge.recordedVotedEntryIDs.isEmpty {
