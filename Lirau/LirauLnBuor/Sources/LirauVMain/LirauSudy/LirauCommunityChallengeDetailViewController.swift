...
final class LirauCommunityChallengeDetailViewController: UIViewController {
    ...
    private func challengeForDisplay() -> LirauCommunityChallenge? {
        ...
        guard challenge.phase == .voting, challenge.profileIntroLorauaCurrentUserMode == .none else {
            return challenge
        }
    ...
    private func makeStatePanel(_ challenge: LirauCommunityChallenge) -> UIView {
        ...
        if challenge.profileIntroLorauaCurrentUserMode == .host {
        ...
        } else if challenge.profileIntroLorauaCurrentUserMode == .submitted {
            stateLabel.text = "You've joined — wait for votes. Your entry is highlighted below, and you can still vote for other learners."
            let shareButton = makeSecondaryButton(title: "Share")
            shareButton.addTarget(self, action: #selector(shareChallenge), for: .touchUpInside)
            buttonRow.addArrangedSubview(shareButton)
        } else if challenge.profileIntroLorauaCurrentUserMode == .voteOnly {
    ...
    private func makeEntryList(_ challenge: LirauCommunityChallenge) -> UIView {
        ...
        let profileIntroLorauaCurrentUser = LirauCommunityUserSnapshot.current
        if challenge.phase == .ended {
            stack.addArrangedSubview(makeWinnerPanel(challenge))
        }

        sortedEntries(for: challenge, profileIntroLorauaCurrentUserID: profileIntroLorauaCurrentUser.userID).forEach { entry in
            let card = LirauCommunityEntryCardView()
            card.configure(entry: entry, challenge: challenge, profileIntroLorauaCurrentUserID: profileIntroLorauaCurrentUser.userID)
    ...
    private func sortedEntries(for challenge: LirauCommunityChallenge, profileIntroLorauaCurrentUserID: String) -> [LirauCommunityEntry] {
        if challenge.phase == .ended {
        ...
        guard let profileIntroLorauaCurrentUserEntryID = challenge.profileIntroLorauaCurrentUserEntryID else {
            return challenge.entries
        }
        return challenge.entries.sorted {
            if $0.id == profileIntroLorauaCurrentUserEntryID { return true }
            if $1.id == profileIntroLorauaCurrentUserEntryID { return false }
            return $0.createdAt > $1.createdAt
        }
    }

    private func makeWinnerPanel(_ challenge: LirauCommunityChallenge) -> UIView {
        let sorted = sortedEntries(for: challenge, profileIntroLorauaCurrentUserID: LirauCommunityUserSnapshot.current.userID)
        let container = UIView()
    ...
    @objc private func joinChallenge() {
        ...
        switch challenge.profileIntroLorauaCurrentUserMode {
        case .none:
    ...
    private func vote(for entry: LirauCommunityEntry) {
        guard let challenge = store.challenge(with: challengeID) else { return }
        let profileIntroLorauaCurrentUser = LirauCommunityUserSnapshot.current
        if challenge.profileIntroLorauaCurrentUserMode == .host || challenge.hostUserID == profileIntroLorauaCurrentUser.userID {
            showToast("Hosts can view entries but can't submit or vote.")
            return
        }
        if challenge.phase == .ended {
        ...
        if entry.userID == profileIntroLorauaCurrentUser.userID {
...
final class LirauCommunityEntryCardView: UIView {
    ...
    func configure(entry: LirauCommunityEntry, challenge: LirauCommunityChallenge, profileIntroLorauaCurrentUserID: String) {
        ...
        let isOwnEntry = entry.userID == profileIntroLorauaCurrentUserID || challenge.profileIntroLorauaCurrentUserEntryID == entry.id
        let isHost = challenge.profileIntroLorauaCurrentUserMode == .host || challenge.hostUserID == profileIntroLorauaCurrentUserID
        ...
        if isHost {
            ...
            voteButton.isEnabled = false
        } else if challenge.status == .speacketLirau || challenge.status == .voting {
