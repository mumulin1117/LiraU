...
@interface LirauProfileHeaderView ()
...
@implementation LirauProfileHeaderView
...
- (void)configureWithUser:(LirauProfileUser *)user {
    self.nameLabel.text = user.displayName.length > 0 ? user.displayName : @"LiraU Learner";
    NSArray *values = @[@(user.followingCount), @(user.postCount), @(user.followersCount)];
    ...
    [self loadAvatar:user.avatarURLString];
}
