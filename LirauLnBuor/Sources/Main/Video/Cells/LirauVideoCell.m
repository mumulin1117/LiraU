...
@implementation LirauVideoCell

+ (NSString *)reuseIdentifier {
    return @"LirauVideoCell";
}
...
- (void)buildView {
    ...
    [videoTapControl addTarget:self action:@selector(didTapVideo) forControlEvents:UIControlEventTouchUpInside];
    [self.contentView addSubview:videoTapControl];

    self.userBadgeView = [[LirauVideoUserBadgeView alloc] init];
    ...
    self.actionBarView = [[LirauVideoActionBarView alloc] init];
...
- (void)configureWithItem:(LirauVideoItem *)item {
    [self.userBadgeView configureWithItem:item];
    self.descriptionLabel.text = [NSString stringWithFormat:@"  %@", [item displayDescription]];
    [self.actionBarView configureWithItem:item];
    self.currentCoverURLString = [item displayCoverURLString];
